...
Вы можете сконфигурировать оригинальный образ `centos` самостоятельно через собственный Dockerfile, так зачем использовать javister-docker-base?
 ...
 * Он экономит время загрузки при передеплое. Docker нужно загрузить базовый образ только один раз. Во время первого деплоя.
   При каждом последующем деплое будут загружены только изменения, сделанные поверх базового образа.
 ...
 * [Администрирование контейнера](#container_administration)
   ...
   * [Вход в контейнер, или запуск команды внутри него, с помощью `docker exec`](#login_docker_exec)
